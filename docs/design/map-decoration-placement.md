...
- 由 `EntityProfileManager` 按建筑类型在对应区间内顺序分配：
  - 房舍类型配置：`10000`、`10001`、`10002`…（`10000` 预留给树，默认房舍为 `10001`）
...
- 旧 `map.json` 中 `decoration` 字段可能仍存 `1/10/11/12`（房舍/房舍/商店子配置），读取时映射到新的 `build_cfg_id`：
  - `1`、`10` → `10001`（房舍）
  - `11`、`12` → `20001`、`20002`（商店类型下的装饰子配置，待后续细化）
- 旧 `debug_panel_config.cfg` 中 Profile ID `10/11/12` 在加载时迁移到 `10001/20001/20002`。
