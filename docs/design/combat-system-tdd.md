...
- **碰撞即开战**：角色移向敌方 30% 后检测碰撞（客户端检测 + 服务端校验），确认后触发简化普攻，随后进入 ATB 循环。详见 [`movement-system.md`](movement-system.md)。
...
> **移动流程、30% 检查点、碰撞预约系统详见 [`movement-system.md`](movement-system.md)。**
> 本节描述碰撞触发后的战斗处理逻辑。
