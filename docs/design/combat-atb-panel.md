...
- ATB 计算在服务端 `map_pool/combat/manager.lua` 中是权威来源
...
### 5.1 map_pool/combat/manager.lua

- 在 `tickATB` 末尾调用 `self:broadcastCombatState(maps)`
- 新增 `broadcastCombatState(maps)`：按玩家聚合单位，构造 `CombatLogNotify` 并发送

### 5.2 map_pool/service.lua

- 复用现有的 `broadcastToMap` / `sendToAccount` 逻辑即可，无需新增 handler
...
| 问题 | 确认结果 |
|------|---------|
| 显示哪些单位？ | 玩家自己 + 所有当前有战斗关系的敌人 |
| ATB 如何映射？ | 左端 = 0，右端 = 100，直接按 atb 值定位 |
| 行动后如何表现？ | 到达 100 执行动作，标记瞬间回到左端（0） |
| 同步方式？ | 服务端每 100ms 推送一次完整 ATB 状态 |
| 脱战后？ | 收到空列表，进度条消失 |
| 标记样式？ | 玩家 = 绿色方块+名字在上；敌人 = 红色圆点+名字在下 |

---

## 7. 下一步任务拆分

1. 更新 `game.proto` 和 `message_id.proto`，重新生成协议代码
2. 服务端 `CombatManager` 插入 `broadcastCombatState`
3. 客户端 `NetworkManager` 解析 `CombatStateNotify`
4. 客户端创建 `CombatATBPanel` 场景与脚本
5. 更新 `main.tscn` 挂载面板
6. 联调测试
