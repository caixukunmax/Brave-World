...
- **一句话描述**：允许策划/开发者在游戏中直接编排剧情、镜头、角色动作与事件触发，降低剧情与演出内容的制作门槛。
- **可能包含**：时间轴编辑器、镜头路径、NPC 走位指令、对话气泡、触发器、预览回放。
- **关联系统**：剧情系统（StoryPanel）、地图编辑器、NPC AI、摄像机控制。
- **下一步**：收集具体使用场景（过场动画？任务演出？开场 CG？），再决定是内置编辑器还是外部工具 + 配置表。
