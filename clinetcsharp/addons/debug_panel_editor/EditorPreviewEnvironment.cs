...
namespace ClinetCSharp.Editor
{
    ...
    {
        /// <summary>
        /// 预览渲染超采样整数倍率（≥1）：编辑器显示缩放 100% → 1；125%/150%/175% → 2。
        /// 用法：SubViewport 以 N× 分辨率渲染、SubViewportContainer.StretchShrink=N 按 1/N 显示，
        /// 保证纹理 texel 密度 ≥ 物理像素密度（等比超采样，不畸变、不欠采样），
        /// 从显示链路上根除「编辑器显示缩放导致的整体模糊」。
        /// </summary>
        ...
        /// <summary>
        /// 一次性把预览容器/视口对齐到游戏渲染环境：N× 超采样渲染 + 游戏主题。
        /// 返回超采样倍率；容器 Resized 时必须再调 SyncViewportSize 同步视口渲染尺寸。
        /// 注意：只设置 Stretch/StretchShrink/Theme，不在此处改 Size——容器构建时尺寸未参与
        /// 布局（2x2），真实尺寸由调用方在 Resized 时同步（见 AGENTS.md SubViewport 条目）。
        /// </summary>
        ...
        /// <summary>按容器当前尺寸同步 SubViewport 渲染尺寸（容器 Resized 时调用）。</summary>
        public static void SyncViewportSize(SubViewportContainer container, SubViewport viewport, int factor)
        {
            if (container == null || viewport == null) return;
            var s = container.Size;
            viewport.Size = new Vector2I(
                Mathf.Max(1, Mathf.CeilToInt(s.X * factor)),
                Mathf.Max(1, Mathf.CeilToInt(s.Y * factor)));
        }
