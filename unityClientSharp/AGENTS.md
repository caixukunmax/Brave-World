...
23. **调试面板 = Editor 程序集的 EditorWindow，不做运行时游戏内面板。** 最终形态 `Window > BraveWorld > 调试面板`（`Assets/Editor/DebugPanel/`，`BraveWorld.Editor` 程序集，不进包）；曾迁移的运行时 UGUI 版（`Assets/Scripts/DebugPanel/`）已废弃删除，不要再往场景里加调试 UI。配置层在 Runtime（`EntityProfileManager`/`JsonProfileStore`），编辑与 Play 共用 `persistentDataPath/entity_profiles.json`：Edit 模式改的配置下次 Play 自动生效；Play 模式改完点面板底部「应用并刷新场景实体」→ `ProfileRefreshUtil.RefreshAll()` 重建实体视觉（`EntityVisualBase/MapDecoration.RefreshFromProfile` 保留血条/标签等运行时状态）。面板 UI 始终代表 Profile 配置数据，禁止用实体运行时状态覆盖配置。
24. **地图编辑器 = Editor 程序集 EditorWindow + Scene 视图画布，不做游戏内编辑器。** `Window > BraveWorld > 地图编辑器`（`Assets/Editor/MapEditor/`），Edit 模式直接编辑 `Assets/StreamingAssets/Data/maps/<图>/map.json`。要点：①编辑会话是 `HideAndDontSave` 隔离根（`MapEditSession`），Edit 模式 Awake 不执行，`GridManager`/`MapRenderer`/`MapDecorationManager` 全部手动初始化（`LoadMap`/`EnsureInit`/`RefreshFrame`/`SpawnDecorations`）；②Edit 模式下 `Object.Destroy` 非法，运行时代码中可能被编辑器触发的销毁一律走 `DestroyEditSafe`（按 `Application.isPlaying` 分支）；③`GridManager.LoadMap` 必须保留 map.json 的 displayName/spawn 并在 `SaveCurrentMap` 原样回写——丢了保存时会被默认值静默覆盖（有回归测试 `SaveCurrentMap_PreservesSpawnAndDisplayName`）；④进 Play 前强制退出编辑会话（隔离对象的 Awake 会污染 `Walkability` 静态）；⑤map.json 保持 schema v3 不变（第 36 行节）；「同步到服务器」= 拷贝 map.json + 更新 `servercsharp/data/map_registry.json`，需重启 GameServer 生效。
